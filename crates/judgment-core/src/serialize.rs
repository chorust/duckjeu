...
impl CanonicalState {
    /// Version of the normalization rules used to produce this state.
    ...
    /// Self-describing bytes for identity/indexing: magic, version, state kind, payload length,
    /// then the full canonical payload. Provider request serialization remains unchanged.
    pub fn versioned_canonical_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::with_capacity(17 + self.canonical.len());
        bytes.extend_from_slice(b"DJST");
        bytes.extend_from_slice(&self.canonical_version.to_be_bytes());
        bytes.push(match self.kind {
            StateKind::Text => 1,
            StateKind::Struct => 2,
        });
        bytes.extend_from_slice(&(self.canonical.len() as u64).to_be_bytes());
